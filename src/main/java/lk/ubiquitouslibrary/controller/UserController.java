...
import lk.ubiquitouslibrary.config.Constants;
...
//            mailService.sendCreationEmail(newUser);
     ...
     * @throws RuntimeException {@code 400 (Bad Request)} if the email and login is already in use.
     */
    @PutMapping("/users")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
...
//        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
    ...
    public ResponseEntity<AdminUserDTO> getUser(@PathVariable @Pattern(regexp = Constants.LOGIN_REGEX) String login) {
        ...
        return userService.getUserWithAuthoritiesByLogin(login).map(AdminUserDTO::new).map(ResponseEntity::ok).orElseGet(()->ResponseEntity.notFound().build());
    }
